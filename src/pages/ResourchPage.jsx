...
function MetricCard({ label, value, loading }) {
  return (
    <div className="research-metric">
      <span className="research-metric__value">{loading ? '...' : value ?? 0}</span>
...
export default function ResearchPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setModal({ open: true, title: 'Erro', message: 'Não foi possível carregar os dados.', variant: 'error' })
    } finally {
    ...
    <div className="research-page">
      <div className="research-card">
        ...
        <div className="research-page__header">
          ...
          <p className="research-page__subtitle">Análise de uso dos agentes e interações</p>
        </div>
        ...
        <div className="research-metrics">
          <MetricCard label="Total de interações" value={data.totalInteracoes} loading={loading} />
          <MetricCard label="Alunos ativos" value={data.totalAlunosAtivos} loading={loading} />
          <MetricCard label="Média por aluno" value={data.mediaInteracoesPorAluno} loading={loading} />
        </div>
          ...
          {dataAgente.map(a => (
            <div key={a.key} className="research-agente-card">
              <div className="research-agente-card__header">
                <span className="research-agente-card__nome" style={{ color: AGENTE_COR[a.key] }}>
                  {a.name}
                </span>
                <span className="research-agente-card__pct">{a.percentual}%</span>
              </div>
              <div className="research-progress">
                <div
                  className="research-progress__bar"
                  style={{ width: `${a.percentual}%`, background: AGENTE_COR[a.key] }}
                />
              </div>
              <div className="research-agente-card__footer">
                <span>{a.total} interações</span>
                <span className="research-satisfaction">👍 {a.likes} &nbsp; 👎 {a.dislikes}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="research-charts">
          <div className="research-chart-block">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={dataJogo} layout="vertical" margin={{ top: 0, right: 24, left: 16, bottom: 0 }}>
                <XAxis type="number" tick={{ fontSize: 12, fill: '#777' }} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 13, fill: '#333' }} width={72} />
                <Tooltip
                  formatter={(v) => [`${v} interações`, 'Total']}
                  contentStyle={{ borderRadius: 8, fontSize: 13 }}
                />
                <Bar dataKey="total" radius={[0, 6, 6, 0]} shape={BarShape(CORES_JOGO)} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="research-chart-block">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={dataFuncao} layout="vertical" margin={{ top: 0, right: 24, left: 16, bottom: 0 }}>
                <XAxis type="number" tick={{ fontSize: 12, fill: '#777' }} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 12, fill: '#333', fontFamily: 'monospace' }} width={80} />
                <Tooltip
                  formatter={(v) => [`${v} interações`, 'Total']}
                  contentStyle={{ borderRadius: 8, fontSize: 13 }}
                />
                <Bar dataKey="total" radius={[0, 6, 6, 0]} shape={BarShape(CORES_FUNCAO)} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
